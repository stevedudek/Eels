        ...
        while self.running:
            try:
                ...
                if delay:
                    ...
                    self.model.go() # Send the light data for next frame
                    self.model.send_delay(adj_delay)    # Send the delay amount

                    time.sleep(adj_delay)  # Delay on this end: The only delay!
...
class EelServer(object):
    def __init__(self, eel_model, eel_simulator, args):
        self.args = args
        self.eel_model = eel_model
        self.eel_simulator = eel_simulator
    ...
    def _create_services(self):
        ...
        # Show runner
        self.runner = ShowRunner(self.eel_model, self.eel_simulator, self.queue, args.max_time)
    ...
    def start(self):
        if self.running:
            print "start() called, but eel is already running!"
            return
    ...
    def go_web(self, app):
        ...
        cherrypy.quickstart(EelWeb(app), '/', config=config)
...
if __name__=='__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Eels Light Control')
    ...
    print "Using Eel Simulator at %s:%d" % (sim_host, sim_port)

    from model.simulator import SimulatorModel
    model = SimulatorModel(sim_host, port=sim_port)
    full_eels = eel.load_eels(model)

    app = EelServer(full_eels, model, args)
    try:
        ...
        print "Unhandled exception running Eels!"
        traceback.print_exc()
    finally:
        app.stop()
